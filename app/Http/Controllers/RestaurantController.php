   ...
   public function getOrdersRestaurant(Restaurant $restaurant, Request $request)
{
    ...
    $cashier = $restaurant->cashiers()->create([]);
     ...
     $frontend = config('app.frontend_url', 'http://localhost:3000');
       ...
       // أو أي رابط خاص بالمطعم
        ...
        $qrImage = 
        QrCode::format('png')->merge($logoFullPath, 0.25, true) ->size(400)->errorCorrection('H')->generate($qrData);
   ...
   $cashierQr = $frontend . '/cashier/' . $cashier->id . '?restaurant=' . $restaurant->id . '&user=' . $user_id . '&token=' . $cashier->token;
    ...
    Storage::disk('public')->put($kitchenQrPath, QrCode::format('png')->merge($logoFullPath, 0.25, true) // ← النسبة دي لتصغير اللوجو (25% من حجم الكود)
    ->size(400)
    ->errorCorrection('H')->generate($kitchenQr));
    Storage::disk('public')->put($cashierQrPath, QrCode::format('png')->merge($logoFullPath, 0.25, true) // ← النسبة دي لتصغير اللوجو (25% من حجم الكود)
    ->size(400)
